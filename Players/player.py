...
class Player():
    ...
    def __init__(self, position, controls, color, player_id):
        ...
        self.color = color
        self.rect = pygame.Rect(self.position.x, self.position.y, 32, 64)
    ...
    def collisions(self, platforms):
        ...
        for platform in platforms:
            ...
            if self.rect.colliderect(platform_rect):
                overlap_x = min(self.rect.right - platform_rect.left, platform_rect.right - self.rect.left)
                overlap_y = min(self.rect.bottom - platform_rect.top, platform_rect.bottom - self.rect.top)

                if overlap_x < overlap_y:  # Horizontal collision
                    if self.velocity.x > 0 and self.rect.left < platform_rect.left:  # Moving right into platform
                        self.position.x = platform_rect.left - self.rect.width
                    elif self.velocity.x < 0 and self.rect.right > platform_rect.right:  # Moving left into platform
                    ...
                    elif self.velocity.y < 0 and platform_rect.bottom - self.rect.top <= tolerance:  # Jumping
    ...
    def detect_headbumps(self, platforms):
        self.under_platform = False
        head_zone = pygame.Rect(self.rect.x, self.rect.y - 64, self.rect.width, 32)
        for platform in platforms:
            platform_rect = pygame.Rect(platform.position.x, platform.position.y, platform.dimensions[0], platform.dimensions[1])
